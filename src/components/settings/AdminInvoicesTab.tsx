...
export function AdminInvoicesTab({ showToast, scope = "internal" }: AdminInvoicesTabProps) {
  ...
  const isExternalInvoice = (inv: Invoice) => {
    ...
    // ALL invoices generated by Employees (EMP-* ID, Employee role/designation, Shift/Timesheet sync) strictly belong to Internal Team
    if (cust.startsWith("EMP-") || busAddr.includes("employee") || (inv as any).shiftAttendance || (inv as any).timesheetEntries) {
      return false;
    }

    const billedTo = (inv.billedToName || "").toLowerCase();
    const busName = (inv.businessName || "").toLowerCase();
    return cust.startsWith("EXT-") || (billedTo.includes("external") && !cust.startsWith("EMP-")) || busName.includes("external contractor") || busAddr.includes("vendor");
  };
  ...
  const patchInvoice = async (invoiceId: string, body: Record<string, unknown>) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setInvoices((prev) =>
          prev.map((inv) => ((inv._id || inv.id) === invoiceId ? { ...inv, ...data.invoice } : inv))
        );
        if (viewInvoice && (viewInvoice._id || viewInvoice.id) === invoiceId) {
          setViewInvoice((prev) => prev ? { ...prev, ...data.invoice } : prev);
        }
        showToast(`Invoice status updated to "${data.invoice.status}"`, "success");
      } else {
